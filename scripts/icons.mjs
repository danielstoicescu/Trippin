...
thermostat thumb_up thunderstorm tips_and_updates train umbrella undo water_drop waves weather_snowy wb_twilight wine_bar mobile mobile_share mobile_arrow_down search add_location_alt arrow_back travel_explore timer straighten flag more_horiz tune today pin_drop directions_car history north_east layers my_location warning key local_florist location_on attractions local_mall edit_calendar bookmark flight_takeoff luggage bookmark_add favorite expand_less directions navigation cookie event chevron_right arrow_forward done_all verified explore_nearby hotel`.split(/\s+/).filter(Boolean);
